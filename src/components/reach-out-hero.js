...
const ReachOutHero = () => {
  ...
  return (
    <div
      className={`relative w-full h-[100vh] bg-cover bg-center ${styles.hero}`}
    >
      <div
        className={`absolute top-[200px] left-10 transform transition-all duration-1000 ${titleVisible ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'}`}
      >
        <h1 className="text-6xl font-bold text-black">Reach Out</h1>
      </div>
      <div
        className={`absolute bottom-[502px] left-[150px] transition-opacity duration-1000 ${sentimentVisible ? 'opacity-100' : 'opacity-0'}`}
      >
